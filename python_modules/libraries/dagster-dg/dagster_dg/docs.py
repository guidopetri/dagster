...
def _get_commented_obj(
    name: str,
    json_schema: Mapping[str, Any],
    subschema: Mapping[str, Any],
    available_scope: Optional[AbstractSet[str]] = None,
    ...
    if objtype == "object":
        return CommentedObject(
            key=name,
            value={
                k: _get_commented_obj(k, json_schema, v)
...
def generate_sample_yaml(component_type: str, json_schema: Mapping[str, Any]) -> str:
    params_obj = _get_commented_obj("params", json_schema, json_schema)
    return yaml.dump(
...
def markdown_for_component_type(component_type_metadata: RemoteComponentType) -> str:
    component_type_name = f"{component_type_metadata.package}.{component_type_metadata.name}"
    sample_yaml = generate_sample_yaml(
        component_type_name, component_type_metadata.component_params_schema or {}
    )
